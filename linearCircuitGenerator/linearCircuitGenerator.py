...
def getUnique(root):
	...
	for item in root:
		...
		#If the item was not added to the bad list, it is not a duplicate
		if item not in bad:
			#If the item is not in our uniqueList, it is not a duplicate
			if item not in unique:
				#It hasn't been found in the uniqueList yet so proceed to check it further.
				found = False
				#Loop through the unique list and compare it with the recursive function just to be sure
				for u in unique:
					#This is the costly recursive function. It will call itself on the same two lists repeatedly until it determines if the two lists match.
					#This step is required until listSort() can unambiguously sort configurations past 6 resistors, this is not possible yet
					if checkItems(item, u):
						#We not only append the item, we append it with the item it failed against so that we may analyze what is happening.
						bad.append(item)
						
						#We found it inside the uniqueList, it is a duplicate
						found = True
						break
